...
function extractColumnHeaders(records: any[]) {
  if (!records || records.length === 0) return [];
  const firstRecord = records[0];
  return Object.keys(firstRecord);
}

function formatRecordsForAI(records: any[], headers: string[]) {
  if (!records || records.length === 0) return '';
...
async function processBatchWithAI(batch: any[], headers: string[], apiKey: string) {
  if (!apiKey) {
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to process records' }, { status: 500 });
  }
}
